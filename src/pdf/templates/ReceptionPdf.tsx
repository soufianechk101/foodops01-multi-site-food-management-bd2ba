...
export interface ReceptionPdfLine {
  code: string;
  name: string;
  unit: string;
  ordered: number;
    ...
    <div className="pdf-document">
      ...
      >
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: model.poNumber ? '1fr 1fr 1fr' : '1fr 1fr',
            gap: '16px',
            fontSize: '9pt',
          }}
        >
      ...
      >
        <thead>
          <tr style={{ background: '#1b503b', color: 'white' }}>
            ...
            <th style={{ textAlign: 'left', padding: '8px 6px', fontWeight: 600, fontSize: '8pt', textTransform: 'uppercase', letterSpacing: '0.05em', border: 'none' }}>Article</th>
            ...
            <th style={{ textAlign: 'right', padding: '8px 6px', fontWeight: 600, fontSize: '8pt', textTransform: 'uppercase', letterSpacing: '0.05em', border: 'none' }}>Commandé</th>
            <th style={{ textAlign: 'right', padding: '8px 6px', fontWeight: 600, fontSize: '8pt', textTransform: 'uppercase', letterSpacing: '0.05em', border: 'none' }}>Reçu</th>
        ...
        <tbody>
          {model.lines.map((line, idx) => {
            const diff = line.received - line.ordered;
            const receivedColor = diff > 0 ? '#1e7a4e' : diff < 0 ? '#b8432a' : '#1a2620';
            return (
              <tr key={idx} style={{ background: idx % 2 === 0 ? 'white' : '#f9fafb' }}>
                ...
                <td style={{ padding: '7px 6px', textAlign: 'right', fontFamily: 'IBM Plex Mono, monospace', color: '#43554b', borderTop: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}>
                  {fmtNum(line.ordered)}
                </td>
                <td style={{ padding: '7px 6px', textAlign: 'right', fontFamily: 'IBM Plex Mono, monospace', fontWeight: 600, color: receivedColor, borderTop: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}>
                  {fmtNum(line.received)}
                </td>
        ...
        <tfoot>
          <tr style={{ background: '#f3f4f6', fontWeight: 600 }}>
            <td colSpan={6} style={{ padding: '10px 6px', textAlign: 'right', fontSize: '10pt', borderLeft: '1px solid #d1d5db', borderTop: '2px solid #1b503b', borderBottom: '1px solid #d1d5db' }}>
              Total HT
            </td>
      ...
      <div className="pdf-signatures" style={{ marginTop: '40px', paddingTop: '20px', borderTop: '1px solid #e5e7eb' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '24px', fontSize: '9pt' }}>
          <div>
            <p style={{ fontWeight: 600, marginBottom: '40px', paddingBottom: '6px', borderBottom: '1px solid #1a2620' }}>
              Réceptionné par
            </p>
            <p style={{ fontSize: '8pt', color: '#7c8c81', margin: 0 }}>
      ...
      <div className="pdf-footer" style={{ marginTop: '40px', paddingTop: '10px', borderTop: '1px solid #d1d5db', fontSize: '8pt', color: '#7c8c81', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span>{model.company.name} — Bon de Réception N° {model.number}</span>
